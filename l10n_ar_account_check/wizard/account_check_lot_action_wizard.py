...
class AccountCheckLotActionWizard(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if self._context.get('default_action_type') in ['used', 'deposited', 're_deposited']:
            ...
            if self._context.get('default_action_type') in ['re_deposited'] and checks:
                rec['journal_id'] = checks[0].deposited_journal_id.id
    ...
    def action_confirm(self):
        self.ensure_one()
        if self.action_type not in ['bank_debit', 'used', 'negotiated', 'selled', 'deposited', 're_deposited']:
            raise ValidationError(_(
